...
public class MutipleJob implements Callable<String> {
	...
	public MutipleJob(String urlmode, int task_count, NodeFilter filter, TaskManager manager) {
	...
	public String call() throws Exception {
		...
		for (int i = 1; i <= task_count; i++) {
			resualt.add(manager.runTask(new HtmlPaserJob(urlmode.replace(mark, String.valueOf(i)), filter)));
		...
		while (true) {
			...
			while (iterator.hasNext()) {
				...
				try {
					...
					r.cancel(true);
					// 重新添加任务
					resualt.set(index, manager.runTask(new HtmlPaserJob(urlmode.replace(mark, String.valueOf(index)), filter)));
			...
			if (tempUnDoneList.size() == 0) {
				...
				log.error("task left 【" + tempUnDoneList.size() + "】 run again...");
			}
		}
